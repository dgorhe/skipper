...
if (nrow(enriched_window_data) == 0) {
    message("No enriched windows found. Creating empty output file.")
    # Create an empty tibble with the correct structure
    reproducible_enriched_window_data <- tibble(
        chr = character(),
        start = numeric(),
        end = numeric(),
        name = character(),
        score = numeric(),
        strand = character(),
        feature_type_top = character(),
        input_sum = numeric(),
        clip_sum = numeric(),
        enrichment_n = numeric(),
        enrichment_l2or_min = numeric(),
        enrichment_l2or_mean = numeric(),
        enrichment_l2or_max = numeric(),
        p_max = numeric(),
        p_min = numeric(),
        q_max = numeric(),
        q_min = numeric()
...
reproducible_enriched_window_data <- enriched_window_data %>%
    ...
    filter(enrichment_n > 1) %>%
    arrange(enrichment_l2or_mean %>% desc())
...
reproducible_enriched_window_data %>%
    ...
    ggplot(aes(feature_type_top, fill = feature_group, group = feature_type_top)) +
    theme_bw(base_size = 7) +
    geom_bar() +
    theme(legend.position = "none", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
    xlab("Type of feature") +
    ylab("# enriched")
dev.off()
...
reproducible_enriched_window_data %>%
    mutate(feature_group = sub("_.*", "", feature_type_top)) %>%
    group_by(feature_group, feature_type_top) %>%
    count() %>%
    ggplot(aes(feature_type_top, n, color = feature_group, group = feature_type_top)) +
    theme_bw(base_size = 7) +
    geom_point(stroke = 0) +
    theme(legend.position = "none", axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)) +
    xlab("Type of feature") +

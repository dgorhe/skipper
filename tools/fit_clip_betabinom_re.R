...
clip_betabinom_re_fit_data <- lapply(other_clip_replicates, function(other_clip_replicate) {
    ...
    p_data_null <- nonzero_re_data %>%
        group_by(.data[[given_clip_replicate]], .data[[other_clip_replicate]]) %>%
        summarize() %>%
        mutate(enrichment_l2or = log2((.data[[given_clip_replicate]] + betabinom_coefs["mu"]) / (.data[[other_clip_replicate]] + 1 - betabinom_coefs["mu"])) - log2(betabinom_coefs["mu"] / (1 - betabinom_coefs["mu"]))) %>%
    ...
    label_data <- q_data_null %>%
        arrange(qvalue) %>%
        filter(!grepl("\\)n$", repeat_name)) %>%
        head(4)
    ...
    betabinom_coefs %>%
        ...
        mutate(given_clip_fraction = given_clip_fraction, given_replicate = given_clip_replicate, other_replicate = other_clip_replicate, experiment = experiment)
}) %>% bind_rows()

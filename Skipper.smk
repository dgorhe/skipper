...
# example command
# snakemake -kps Skipper.py -w 25 -j 30 --cluster "qsub -e
# {params.error_file} -o {params.out_file} -l walltime={params.run_time}
# -l nodes=1:ppn={threads} -q home-yeo"
...
manifest = pd.read_csv(MANIFEST, comment="#", index_col=False).dropna(
    subset=["Experiment", "Sample"]
)
manifest["CLIP_replicate"] = pd.to_numeric(manifest.CLIP_replicate, downcast="integer")
manifest["Input_replicate"] = pd.to_numeric(
    manifest.Input_replicate, downcast="integer"
)
...
if max(manifest.groupby("Sample")["Input_replicate"].agg(lambda x: min(x))) > 1:
    raise Exception(
        "Input replicates for samples in manifest do not increment from 1 as expected"
    )

if max(manifest.groupby("Sample")["CLIP_replicate"].agg(lambda x: min(x))) > 1:
    raise Exception(
        "CLIP replicates for samples in manifest do not increment from 1 as expected"
    )

manifest["Input_replicate_label"] = [
    (str(sample) + "_IN_" + str(replicate)).replace(" ", "")
    for replicate, sample in zip(
        manifest.Input_replicate.tolist(), manifest.Sample.tolist()
    )
]
manifest["CLIP_replicate_label"] = [
    (str(sample) + "_IP_" + str(replicate)).replace(" ", "")
    for replicate, sample in zip(
        manifest.CLIP_replicate.tolist(), manifest.Sample.tolist()
    )
]

input_replicates = manifest.loc[
    :,
    manifest.columns.isin(
        ["Input_replicate_label", "Input_fastq", "Input_bam", "Input_adapter"]
    ),
].drop_duplicates()
clip_replicates = manifest.loc[
    :,
    manifest.columns.isin(
        ["CLIP_replicate_label", "CLIP_fastq", "CLIP_bam", "CLIP_adapter"]
    ),
].drop_duplicates()

if len(input_replicates) != len(
    input_replicates[["Input_replicate_label"]].drop_duplicates()
) or len(clip_replicates) != len(
    clip_replicates[["CLIP_replicate_label"]].drop_duplicates()
):
...
if all(bam in manifest.columns.tolist() for bam in ["Input_bam", "CLIP_bam"]):
    replicate_label_to_bams = dict(
        zip(
            input_replicate_labels + clip_replicate_labels,
            input_replicates.Input_bam.tolist() + clip_replicates.CLIP_bam.tolist(),
        )
    )
else:
    replicate_label_to_bams = dict(
        zip(
            input_replicate_labels + clip_replicate_labels,
            [
                "output/bams/dedup/genome/"
                + replicate_label
                + ".genome.Aligned.sort.dedup.bam"
                for replicate_label in input_replicate_labels + clip_replicate_labels
            ],
        )
    )

experiment_labels = pd.Series(manifest.Experiment.drop_duplicates().tolist())
experiment_data = manifest.groupby("Experiment").agg(
    {"CLIP_replicate_label": list, "Input_replicate_label": list}
)

if "Input_fastq" in manifest.columns:
    replicate_label_to_fastqs = dict(
        zip(
            input_replicate_labels + clip_replicate_labels,
            input_replicates.Input_fastq.tolist() + clip_replicates.CLIP_fastq.tolist(),
        )
    )
    replicate_label_to_adapter = dict(
        zip(
            input_replicate_labels + clip_replicate_labels,
            input_replicates.Input_adapter.tolist()
            + clip_replicates.CLIP_adapter.tolist(),
        )
    )

overdispersion_replicate_lookup = dict(
    zip(
        manifest.CLIP_replicate_label.tolist(),
        (
            manifest.Input_replicate_label.tolist()
            if OVERDISPERSION_MODE == "input"
            else manifest.CLIP_replicate_label.tolist()
        ),
    )
)
clip_to_input_replicate_label = dict(
    zip(manifest.CLIP_replicate_label.tolist(), manifest.Input_replicate_label.tolist())
)
experiment_to_replicate_labels = dict(
    zip(
        experiment_data.index.tolist(),
        [
            reduce(lambda agg, x: agg if x in agg else agg + [x], inputs, []) + clips
            for inputs, clips in zip(
                experiment_data.Input_replicate_label,
                experiment_data.CLIP_replicate_label,
            )
        ],
    )
)
experiment_to_clip_replicate_labels = dict(
    zip(experiment_data.index.tolist(), experiment_data.CLIP_replicate_label)
)

experiment_to_input_replicate_labels = {}
for experiment_label, label_list in zip(
    experiment_data.index, experiment_data.Input_replicate_label
):
    ...
    for entry in label_list:
        ...
        experiment_to_input_replicate_labels[experiment_label].update(
            {entry: list(replicates)}
        )

if (
    os.path.exists("installation/UMICollapse-1.0.0/umicollapse.jar")
    and os.path.exists("installation/UMICollapse-1.0.0/lib/htsjdk-2.19.0.jar")
    and os.path.exists("installation/UMICollapse-1.0.0/lib/snappy-java-1.1.7.3.jar")
):
...
rule all:
    input:
        expand(
            "output/fastqc/initial/{replicate_label}_fastqc.html",
            replicate_label=replicate_labels,
        ),
        expand(
            "output/fastqc/processed/{replicate_label}.trimmed.umi_fastqc.html",
            replicate_label=replicate_labels,
        ),
        expand(
            "output/bams/dedup/genome/{replicate_label}.genome.Aligned.sort.dedup.bam",
            replicate_label=replicate_labels,
        ),
        expand(
            "output/bams/dedup/genome/{replicate_label}.genome.Aligned.sort.dedup.bam.bai",
            replicate_label=replicate_labels,
        ),
        expand(
            "output/bigwigs/unscaled/plus/{replicate_label}.unscaled.plus.bw",
            replicate_label=replicate_labels,
        ),
        expand(
            "output/bigwigs/scaled/plus/{replicate_label}.scaled.plus.bw",
            replicate_label=replicate_labels,
        ),
        expand(
        ...
        expand(
            ...
            clip_replicate_label=manifest.CLIP_replicate_label,
        ),
        expand(
        ...
        expand(
            "output/counts/repeats/tables/family/{experiment_label}.tsv.gz",
            experiment_label=manifest.Experiment,
        ),
        expand(
        ...
        expand(
            "output/finemapping/mapped_sites/{experiment_label}.finemapped_windows.bed.gz",
            experiment_label=manifest.Experiment,
        ),
        expand(
            "output/homer/finemapped_results/{experiment_label}/homerResults.html",
            experiment_label=manifest.Experiment,
        ),
        expand(
            "output/gene_sets/{experiment_label}.enriched_terms.tsv.gz",
            experiment_label=manifest.Experiment,
        ),
...
rule run_initial_fastqc:
    input:
        fq=lambda wildcards: replicate_label_to_fastqs[
            wildcards.replicate_label
        ].split(" "),
    output:
    ...
    log:
        "logs/fastqc/unassigned_experiment.{replicate_label}.initial_fastqc.log",
    threads: 1
    params:
    ...
    benchmark:
        "benchmarks/fastqc/unassigned_experiment.{replicate_label}.initial_fastqc.txt"
    container:
...
rule trim_fastq:
    ...
    log:
        "logs/trim/unassigned_experiment.{replicate_label}.trim.log",
    threads: 8
    params:
        run_time="4:00:00",
        memory="15000",
        ...
        job_name="trim_fastq",
    benchmark:
        "benchmarks/trim/unassigned_experiment.{replicate_label}.trim.txt"
    container:
...
rule extract_umi:
    ...
    log:
        "logs/umi/unassigned_experiment.{replicate_label}.extract_umi.log",
    threads: 8
    params:
        ...
        umi_length=UMI_SIZE,
    benchmark:
        "benchmarks/umi/unassigned_experiment.{replicate_label}.extract_umi.txt"
    container:
...
rule run_trimmed_fastqc:
    ...
    log:
        "logs/fastqc/unassigned_experiment.{replicate_label}.trimmed_fastqc.log",
    threads: 1
    params:
    ...
    benchmark:
        "benchmarks/fastqc/unassigned_experiment.{replicate_label}.trimmed_fastqc.txt"
    container:
        "docker://howardxu520/skipper:fastqc_0.12.1"
    shell:
        """
...
rule align_reads:
    ...
    log:
        "logs/align/unassigned_experiment.{replicate_label}.align_reads_genome.log",
    threads: 8
    params:
        ...
        rg="{replicate_label}",
    benchmark:
        "benchmarks/align/unassigned_experiment.{replicate_label}.align_reads_genome.txt"
    container:
...
rule sort_bam:
    ...
    log:
        "logs/sort/{ref}/unassigned_experiment.{replicate_label}.sort_bam.log",
    threads: 2
    params:
        ...
        job_name="sortbam",
    benchmark:
        "benchmarks/sort/{ref}/unassigned_experiment.{replicate_label}.sort_bam.txt"
    container:
...
rule index_bams:
    ...
    log:
        "logs/index_bam/{round}/{ref}/{mid}/unassigned_experiment.{replicate_label}.index_bam.log",
    threads: 2
    params:
        ...
        job_name="index_bam",
    benchmark:
        "benchmarks/index_bam/{round}/{ref}/{mid}/unassigned_experiment.{replicate_label}.index_bam.txt"
    container:
        "docker://howardxu520/skipper:samtools_1.17_bedtools_2.31.0"
    shell:
        """
...
rule dedup_umi:
    ...
    log:
        "logs/dedup/genome/unassigned_experiment.{replicate_label}.dedup_umi.log",
    params:
    ...
    benchmark:
        "benchmarks/dedup/genome/unassigned_experiment.{replicate_label}.dedup_umi.txt"
    container:
...
rule make_unscaled_bigwig:
    ...
    output:
        bg_plus=temp(
            "output/bedgraphs/unscaled/plus/{replicate_label}.unscaled.plus.bg"
        ),
        bg_minus=temp(
            "output/bedgraphs/unscaled/minus/{replicate_label}.unscaled.minus.bg"
        ),
    ...
    log:
        "logs/bigwigs/unassigned_experiment.{replicate_label}.make_bigwig.log",
    params:
    ...
    benchmark:
        "benchmarks/bigwigs/unassigned_experiment.{replicate_label}.make_bigwig.txt"
    container:
...
rule make_scaled_bigwig:
    ...
    log:
        "logs/bigwigs/unassigned_experiment.{replicate_label}.make_scaled_bigwig.log",
    params:
        error_file="stderr/{replicate_label}.make_bigwig.err",
        out_file="stdout/{replicate_label}.make_bigwig.out",
        run_time="40:00",
...
rule uniq_repeats:
    ...
    log:
        "logs/uniq_repeats/uniq_repeats.log",
    params:
...
rule quantify_repeats:
    ...
    log:
        "logs/repeats/unassigned_experiment.{replicate_label}.quantify_repeats.log",
    params:
        ...
        prefix="output/bams/dedup/genome/{replicate_label}.genome.sort",
    benchmark:
        "benchmarks/repeats/unassigned_experiment.{replicate_label}.quantify_repeats.txt"
    container:
        "docker://howardxu520/skipper:bedtools_2.31.0"
    shell:
        """
...
rule make_repeat_count_tables:
    ...
    log:
        "logs/counts/{experiment_label}.all_replicates.make_repeat_count_table.log",
    params:
...
rule fit_clip_betabinomial_re_model:
    ...
    log:
        "logs/fit_clip_betabinomial_re_model/{experiment_label}.{clip_replicate_label}.fit_clip.log",
    params:
    ...
    benchmark:
        "benchmarks/fit_clip_betabinomial_re_model/{experiment_label}.{clip_replicate_label}.fit_clip.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule fit_input_betabinomial_re_model:
    ...
    log:
        "logs/fit_input_betabinomial_re_model/{experiment_label}.{input_replicate_label}.fit_input.log",
    params:
    ...
    benchmark:
        "benchmarks/fit_input_betabinomial_re_model/{experiment_label}.{input_replicate_label}.fit_input.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule call_enriched_re:
    input:
        table="output/counts/repeats/tables/name/{experiment_label}.tsv.gz",
        replicate=lambda wildcards: "output/counts/repeats/vectors/"
        + re.sub("IP_\\d$", "IP_2", wildcards.clip_replicate_label)
        + ".counts",
        repeats=REPEAT_BED,
        parameters=lambda wildcards: "output/"
        + OVERDISPERSION_MODE
        + "_model_coef_re/{experiment_label}."
    ...
    log:
        "logs/call_enriched_re/{experiment_label}.{clip_replicate_label}.call_enriched_re.log",
    params:
        input_replicate_label=lambda wildcards: clip_to_input_replicate_label[
    ...
    benchmark:
        "benchmarks/call_enriched_re/{experiment_label}.{clip_replicate_label}.call_enriched_re.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule find_reproducible_enriched_re:
    input:
        windows=lambda wildcards: expand(
            "output/enriched_re/{{experiment_label}}.{clip_replicate_label}.enriched_re.tsv.gz",
            clip_replicate_label=experiment_to_clip_replicate_labels[
    ...
    log:
        "logs/find_reproducible_enriched_re/{experiment_label}.all_replicates.reproducible.log",
    params:
    ...
    benchmark:
        "benchmarks/find_reproducible_enriched_re/{experiment_label}.all_replicates.reproducible.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule partition_bam_reads:
    ...
    log:
        "logs/counts/unassigned_experiment.{replicate_label}.partition_bam_reads.log",
    params:
    ...
    benchmark:
        "benchmarks/counts/unassigned_experiment.{replicate_label}.partition_bam_reads.txt"
    container:
        "docker://howardxu520/skipper:bedtools_2.31.0"
    shell:
        """
...
rule calc_partition_nuc:
    ...
    log:
        "logs/calc_partition_nuc/calc_partition_nuc.log",
    params:
        error_file="stderr/calc_partition_nuc.err",
        out_file="stdout/calc_partition_nuc.out",
        run_time="00:10:00",
...
rule make_genome_count_table:
    ...
    log:
        "logs/counts/{experiment_label}.all_replicates.make_genome_count_table.log",
    params:
    ...
    benchmark:
        "benchmarks/counts/{experiment_label}.all_replicates.make_genome_count_table.txt"
    container:
        "docker://howardxu520/skipper:bedtools_2.31.0"
    shell:
        """
...
rule fit_input_betabinomial_model:
    ...
    log:
        "logs/betabinomial/{experiment_label}.{input_replicate_label}.fit_input.log",
    params:
    ...
    benchmark:
        "benchmarks/betabinomial/{experiment_label}.{input_replicate_label}.fit_input.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule fit_clip_betabinomial_model:
    ...
    log:
        "logs/fit_clip_betabinomial_model/{experiment_label}.{clip_replicate_label}.fit_clip.log",
    params:
    ...
    benchmark:
        "benchmarks/fit_clip_betabinomial_model/{experiment_label}.{clip_replicate_label}.fit_clip.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule call_enriched_windows:
    input:
        ...
        replicate=lambda wildcards: "output/counts/genome/vectors/"
        + re.sub("IP_\\d$", "IP_2", wildcards.clip_replicate_label)
        + ".counts",
        table="output/counts/genome/tables/{experiment_label}.tsv.gz",
        parameters=lambda wildcards: "output/"
        + OVERDISPERSION_MODE
        + "_model_coef/{experiment_label}."
        + overdispersion_replicate_lookup[wildcards.clip_replicate_label]
        + ".tsv",
    output:
    ...
    log:
        "logs/call_enriched_windows/{experiment_label}.{clip_replicate_label}.call_enriched_windows.log",
    params:
        input_replicate_label=lambda wildcards: clip_to_input_replicate_label[
            wildcards.clip_replicate_label
        ],
    ...
    benchmark:
        "benchmarks/call_enriched_windows/{experiment_label}.{clip_replicate_label}.call_enriched_windows.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule check_window_concordance:
    ...
    log:
        "logs/check_window_concordance/{experiment_label}.all_replicates.concordance.log",
    params:
    ...
    benchmark:
        "benchmarks/check_window_concordance/{experiment_label}.all_replicates.concordance.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule find_reproducible_enriched_windows:
    ...
    log:
        "logs/find_reproducible_enriched_windows/{experiment_label}.all_replicates.reproducible.log",
    params:
    ...
    benchmark:
        "benchmarks/find_reproducible_enriched_windows/{experiment_label}.all_replicates.reproducible.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule sample_background_windows_by_region:
    ...
    log:
        "logs/sample_background_windows_by_region/{experiment_label}.sample_background_windows_by_region.log",
    params:
    ...
    benchmark:
        "benchmarks/sample_background_windows_by_region/{experiment_label}.sample_background_windows_by_region.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule get_nt_coverage:
    input:
        ...
        clip_bams=lambda wildcards: [
            replicate_label_to_bams[clip_replicate_label]
            for clip_replicate_label in experiment_to_clip_replicate_labels[
                wildcards.experiment_label
            ]
        ],
        input_bams=lambda wildcards: [
            replicate_label_to_bams[input_replicate_label]
            for input_replicate_label in experiment_to_input_replicate_labels[
                wildcards.experiment_label
            ]
        ],
    output:
        nt_census=temp(
            "output/finemapping/nt_coverage/{experiment_label}.nt_census.bed"
        ),
        nt_input_counts=temp(
            "output/finemapping/nt_coverage/{experiment_label}.nt_coverage.input.counts"
        ),
        nt_clip_counts=temp(
            "output/finemapping/nt_coverage/{experiment_label}.nt_coverage.clip.counts"
        ),
        nt_coverage="output/finemapping/nt_coverage/{experiment_label}.nt_coverage.bed",
    log:
        "logs/get_nt_coverage/{experiment_label}.all_replicates.reproducible.log",
    params:
    ...
    benchmark:
        "benchmarks/get_nt_coverage/{experiment_label}.all_replicates.reproducible.txt"
    container:
        "docker://howardxu520/skipper:samtools_1.17_bedtools_2.31.0"
    shell:
        """
...
rule finemap_windows:
    ...
    log:
        "logs/finemap_windows/{experiment_label}.all_replicates.reproducible.log",
    params:
    ...
    benchmark:
        "benchmarks/finemap_windows/{experiment_label}.all_replicates.reproducible.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule run_homer:
    ...
    log:
        "logs/run_homer/{experiment_label}.all_replicates.reproducible.log",
    params:
        ...
        job_name="run_homer",
    benchmark:
        "benchmarks/run_homer/{experiment_label}.all_replicates.reproducible.txt"
    container:
...
rule consult_encode_reference:
    input:
        ...
        encode_references=lambda wildcards: expand(
            ...
            reference=[
                "encode3_feature_summary",
                "encode3_eclip_enrichment",
                "encode3_class_assignment",
            ],
        ),
    output:
    ...
    log:
        "logs/consult_encode_reference/skipper.log",
    params:
    ...
    benchmark:
        "benchmarks/consult_encode_reference/skipper.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """
...
rule consult_term_reference:
    ...
    log:
        "logs/consult_term_reference/{experiment_label}.all_replicates.reproducible.log",
    params:
    ...
    benchmark:
        "benchmarks/consult_term_reference/{experiment_label}.all_replicates.reproducible.txt"
    container:
        "docker://howardxu520/skipper:R_4.1.3_1"
    shell:
        """

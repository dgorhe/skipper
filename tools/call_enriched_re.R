...
selected_re_data <- re_data %>%
    ...
    filter(input + clip > 0) %>%
    inner_join(re_gc) %>%
    mutate(gc_bin = cut_number(gc, 20)) %>%
    group_by(gc_bin) %>%
    mutate(baseline_l2or = median(logitb2((clip + 1) / (clip + input + 2)))) %>%
    ungroup()
...
q_data <- p_data %>%
    mutate(simple = ifelse(grepl("\\)n", repeat_name), "kmer", "element")) %>%
    group_by(above_threshold = input + clip >= 10) %>%
    mutate(qvalue = ifelse(above_threshold, p.adjust(pvalue, "fdr"), NA)) %>%
    ungroup() %>%
    arrange(pvalue, desc(enrichment_l2or))

label_data <- q_data %>%
    filter(!grepl("\\)n$", repeat_name), enrichment_l2or > 2.5) %>%
    head(4)
enrichment_max <- q_data$enrichment_l2or %>%
    abs() %>%
    max()
pdf(paste0("output/figures/clip_scatter_re/", output_stem, ".clip_test_distribution.pdf"), height = 1.8, width = 2.8)
q_data %>%
    replace_na(list(qvalue = 1)) %>%
    filter(clip + input >= 10) %>%
    ...
    scale_color_manual(values = c("#bdbdbd", "#f86808"))
